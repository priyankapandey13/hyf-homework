...
const getAllMeal = function (request, response) {
  console.log("Returns all meal with GET request");
...
const insertMeal = function (request, response) {
  ...
  pool.query("INSERT INTO meal SET ?", values, function (
    error,
    results,
    fields
  ) {
...
const returnMealById = function (request, response) {
  ...
  pool.query("SELECT * FROM meal WHERE id = ?", values, function (
    error,
    results,
    fields
  ) {
...
const updateMealById = function (request, response) {
  ...
  pool.query("UPDATE meal SET ? WHERE id = ?", [values, idval], function (
    error,
    results,
    fields
  ) {
...
const deleteMealById = function (request, response) {
  ...
  pool.query("DELETE FROM meal WHERE id = ?", idval, function (
    error,
    results,
    fields
  ) {
...
const mealMaxPrice = function (request, response) {
  ...
  pool.query("SELECT * FROM meal WHERE price < ?", values, function (
    error,
    results,
    fields
  ) {
...
const mealWithReservation = function (request, response) {
  const values = request.query.availableReservations;

  if (values === "true") {
    pool.query(
      "SELECT *, meal.max_reservations, reservation.number_of_guests from meal join reservation on meal.id = reservation.meal_id where max_reservations > number_of_guests",
...
//  + api/meals?title=Yellow%20lentils%20soup // mealWithTitle
const mealWithTitle = function (request, response) {
  ...
  pool.query("SELECT * FROM meal WHERE title = ?", values, function (
    error,
    results,
    fields
  ) {
...
const mealAfterDate = function (request, response) {
  ...
  pool.query("SELECT * FROM meal WHERE created_date > ?", values, function (
    error,
    results,
    fields
  ) {
